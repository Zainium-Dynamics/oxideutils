...
pub fn apply_reloc(
    ...
    match r_type {
        ...
        elf::R_X86_64_PC32 | elf::R_X86_64_PLT32 | elf::R_X86_64_GOTPCREL | elf::R_X86_64_GOTTPOFF => {
            // PLT32/GOTPCREL/GOTTPOFF: for static link treat like PC32 (S+A-P)
            if offset + 4 > section_data.len() {
                bail!("reloc offset out of range");
            }
            // Read existing implicit addend
            let impl_addend = i32::from_le_bytes(section_data[offset..offset + 4].try_into().unwrap());
            let addend = if a != 0 { a } else { impl_addend as i64 };
            let val = (s as i64)
                .wrapping_add(addend)
                .wrapping_sub(p as i64) as i32;
...
pub fn r_type_from_object(kind: object::RelocationKind, size: u8, encoding: object::RelocationEncoding) -> u32 {
